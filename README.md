...
## LLM 工具：`recommend_album`

也可以让 LLM 自己在对话里调用，不用记命令。比如直接说「给我推荐张专辑」「来点 emo 的专辑」，agent 就会自动调起 `recommend_album` 工具，可选参数 `prompt` 用来描述风格 / 流派 / 情绪 / 年代。

由于推荐流水线（联网搜索 + LLM 抽取 + 文案生成）耗时常达数十秒到一两分钟，超过 AstrBot 对工具调用的 120s 硬超时，工具被设计成**立即返回 ack，后台任务再把专辑送达**：

- agent 第一时间回一句「好的，正在挑选...」
- 几十秒到 ~2 分钟后，sender 把专辑卡片推到当前会话
- 推荐内容会写入会话历史，之后再问「刚才推的什么」LLM 能直接答出来
